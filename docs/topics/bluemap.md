...
~~**方法一：** 登录[控制台](https://lab.seatide.net)，如果服务器正在运行状态，你可以在“服务器控制”面板中找到“世界地图”按钮，点击即可跳转到网页地图。~~

**方法二：** 通过控制台首页获取到服务器最新的 IP 地址后，复制，将其输入到地址栏中，并在末尾加上 `:8100` 端口号回车访问即可。换句话说，如果 `X.X.X.X` 是当前服务器 IP 地址，那么网页地图的地址是 `http://X.X.X.X:8100`。
